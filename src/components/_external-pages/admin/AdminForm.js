...
export default function AdminForm() {
  ...
  const createNoLockUp = async () => {
    ...
    try {
      ...
      var tx = await contract.initialize(stakeToken, rewardToken, isReflection == 1 ? dividendToken : zeroAddress, rewardPerBlock, depositFee, withdrawFee, dexRouter, earnedStakePath, reflectionStakePath, isReflection == 1 ? true : false);
      await tx.wait();
  ...
  const createLPFarmAndDeployNew = async () => {
    console.log([rewardToken, isReflection == 1 ? dividendToken : zeroAddress, rewardPerBlock, depositFee, withdrawFee, lpToken, allocPoint])
    ...
    console.log([rewardToken, isReflection == 1 ? dividendToken : zeroAddress, rewardPerBlock, depositFee, withdrawFee, lpToken, allocPoint]);
    console.log(isReflection == 1 ? true : false);
  ...
  const justCreateLPFarm = async () => {
    ...
    try {
      ...
      contract.on('SubFarmCreated', async (_pid, _allocPoint, _lpToken, _depositFee, _withdrawFee, _withUpdate) => {
        ...
        const farm = await getLPFarmByContractAddress(id, stakingContract);
        const farmId = farm['id'];
        console.log(farm);
        const dbResult = await addSubFarmForLPFarm(id, farmId, {
          id: _pid.toNumber(),
          allocPoint: _allocPoint.toNumber(),
          lpToken: _lpToken,
          depositFee: _depositFee.toNumber(),
          withdrawFee: _withdrawFee.toNumber(),
