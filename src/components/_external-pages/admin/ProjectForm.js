...
//
import { addNewProject } from 'src/actions/firebase';
...
export default function ProjectForm() {
  ...
  const submitProject = async () => {
    ...
    try {
      const result = await addNewProject(projectName, tokenSymbol, tokenAddress, telegram, twitter, website, projectBanner, tokenLogo, description, projectBannerUrl, tokenLogoUrl, state.selectedChain);
      setActionLoading(false);
      if (result) {
        startToast("success", `New project successfully created with ID ${result}.`)
      } else {
        startToast("error", "Add new project did not success!");
      }
    }
    catch (e) {
      setActionLoading(false);
      ...
      <Container maxWidth="lg">
        ...
        >
          ...
          <Container spacing={8} sx={{ p: 5, width: 500, height: 'fit-content', border: '1px solid #F0B90B', boxShadow: 'rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px', borderRadius: 1 }}>
            ...
            <Stack direction="row" spacing={7} paddingTop={2} sx={{ justifyContent: 'space-between' }}>
              <CardButton onClick={submitProject} loading={actionLoading} style={{ width: '100%', fontSize: 25 }}>Submit</CardButton>
            </Stack>
          </Container>
        </Container>
      </Container>
